...
## One more, fixed after the fact

- ~~**`CATEGORY_CONFLICTS` is applied by Rule Time but not Odd One Out.**~~
  I first judged this not worth widening: it is a filler rule ("don't offer
  an animal as a non-match against flowers"), and Odd One Out's questions
  enumerate their categories, so no round looked ambiguous.
