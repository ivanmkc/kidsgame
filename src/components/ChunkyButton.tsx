...
export function ChunkyButton({
  label, silent, color, darkColor, onPress, testID, minWidth = 200, fontSize = 19, paddingVertical = 13,
}: {
  label: string;
  silent?: boolean; // skip the built-in tap sfx
  color: string;
  darkColor: string;
  onPress: () => void;
  testID?: string;
  ...
  return (
    <Pressable
      ...
      onPress={() => { if (!silent) sfx.tap(); onPress(); }}
      testID={testID}
