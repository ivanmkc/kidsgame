...
import { makeFastSlowRound, roundsToWin, getActualSpeed, FastSlowRound } from './logic';
...
export function FastSlowGame({ onHome, difficulty, lang }: Props) {
  const total = roundsToWin(difficulty);
  const rngRef = useRef(makeRng(Math.floor(Math.random() * 1e9)));
  const [round, setRound] = useState<FastSlowRound>(() => makeFastSlowRound(rngRef.current, difficulty));
  const [score, setScore] = useState(0);
  const [phase, setPhase] = useState<'listen' | 'pick'>('listen');
  const [wrongPick, setWrongPick] = useState<'fast' | 'slow' | null>(null);
  const [sparkKey, setSparkKey] = useState(0);
  const won = score >= total;
  useWinLine(won, t(lang, 'win.fastslow' as never));

  const playRound = useCallback((r: FastSlowRound) => {
    stopSequence();
    setPhase('listen');
    playSequence(r.notes, r.bpm, undefined, () => setPhase('pick'));
  }, []);
  ...
  const onPick = (picked: 'fast' | 'slow') => {
    if (won || phase !== 'pick') return;
    const actual = getActualSpeed(round);
    if (picked === actual) {
      sfx.good();
      setSparkKey((k) => k + 1);
      const next = score + 1;
  ...
  return (
    <GameShell
      ...
      subtitle={t(lang, 'shell.fastslow.sub' as never)}
      onBack={onHome}
      lang={lang}
      right={<ScoreChip label={`🎵 ${score}/${total}`} testID="fastslow-score" />}
    >
      ...
      <WinOverlay
        visible={won}
        message={t(lang, 'win.fastslow' as never)}
        onNext={reset}
