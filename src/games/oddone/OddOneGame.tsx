...
export function OddOneGame({ onHome, player }: Props) {
  ...
  const roundsToWin = settings.spotitRounds;
  const count = player?.difficulty === 'hard' ? 9 : player?.difficulty === 'medium' ? 6 : 4;
  const rngRef = useRef(makeRng(Math.floor(Math.random() * 1e9)));
  const [round, setRound] = useState(() => makeOddOneRound(rngRef.current, manifest.spotit.icons, count));
  ...
  const onPick = (idx: number) => {
    ...
    if (idx === round.oddIndex) {
      ...
      setWrongIdx(null);
      if (next < roundsToWin) {
        setRound(makeOddOneRound(rngRef.current, manifest.spotit.icons, count));
      }
    } else {
      setWrongIdx(idx);
  ...
  return (
    <GameShell
      title="Odd One Out"
      subtitle="One of these is not like the others!"
      onBack={onHome}
        ...
        <View style={[styles.grid, { width: cols * tile + (cols - 1) * gap, gap }]}>
          {round.items.map((icon, i) => (
            <Pressable
              key={i}
              onPress={() => onPick(i)}
              testID={`oddone-item-${i}-${icon}`}
              style={({ pressed }) => [
                styles.tile,
                shadows.soft,
                { width: tile, height: tile, transform: [{ rotate: `${((i * 7) % 5) - 2}deg` }] },
