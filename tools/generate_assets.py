...
def save_manifest(m: dict) -> None:
    ...
    if MANIFEST.exists():
        ...
        # This process owns ONLY diff/hidden/spotit. Every other section
        # (stories, future keys) is DISK-authoritative: a start-time
        # snapshot must never clobber what another tool wrote since.
        for key, val in disk.items():
            if key not in ("diff", "hidden", "spotit"):
                m[key] = val
    ...
    def _has_files(e: dict) -> bool:
        paths = [e[k] for k in ("imageA", "imageB", "image") if k in e]
        return all((ASSETS / p).exists() for p in paths)
    for key in ("diff", "hidden"):
        m[key] = [e for e in m[key] if _has_files(e)]
