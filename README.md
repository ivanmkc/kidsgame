...
An Expo (React Native + web) box of **19 kids games** (picture games, word &
number suites in four languages, and tap-the-scene storybooks with generated
action videos), with **all art generated by Nano Banana Pro**, scenes verified
by adversarial audit fleets, and gameplay ground truth guaranteed by
construction. All instructions are spoken aloud — the players can't read yet.

**Play:** https://ivanmkc.github.io/kidsgame/

## Storybook pipeline (the new story style)

Interactive books where choices are **hotspots inside the picture**, tapping
one plays a **Veo clip of the hero doing that action**, choices carry real
consequences (comic "oopsie endings"), and every ending is bespoke to the
path taken. One command generates a whole book from a data spec:

```bash
bash tools/gen_storybook.sh <story_id>
```

Full authoring guide + quality gates: [docs/STORYBOOK_PIPELINE.md](docs/STORYBOOK_PIPELINE.md).
Agent runbook: `.claude/skills/storybook-pipeline/`.
